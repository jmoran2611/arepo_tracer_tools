...
class tracer_file:
    ...
    def __init__(self, path, extended_output=True, Nspecies=5):
        ...
        file1.close()
        self.tracer_dt = np.genfromtxt(path_conf, skip_header=1)[
            1
        ]  # Hacky way of getting dt
        ...
        self.data = {}
        self.data["id"] = np.arange(
            self.NTracer
        )  # Probably set at a differen part of the code
        ...
        if self.followed_tracers != 0:
            if os.path.isfile(os.path.join(path, save_name)):
                while os.path.isfile(os.path.join(path, save_name)):
                    n += 1
                    print(
                        "File {:s} already existing, saving as {:s}".format(
                            save_name, filename + str(n).zfill(2)
                        )
                    )
                    save_name = filename + str(n).zfill(2)
    ...
    def byte_map(self, time_index, val_index):
        ...
        return (
            self.offset
            + time_index * self.bts_per_output
            + 12
            + (val_index - 1) * 4 * self.NTracer
        )
        ...
        for t_index in time_indices:
            ...
            for val in values:
                if val == "pos":
                    ...
                    val_data[val] = np.asarray(
                        [val_data[sp] for sp in self.vals_dict["xnuc"]]
                    ).T
                else:
    ...
    def read_single_tracer(tracer_id, file, val_indices):
        ...
        line = 0
        list_of_indices_to_read = np.array(
            [self.byte_map_per_line(i) + tracer_id for i in val_indices]
        )
    ...
    def follow_tracers(self, ids, vals=["pos"], guess_tps=1001):
        ...
        for j, tracer_id in enumerate(ids):
            ...
            for val in vals:
                if val == "pos":
                    tracer[tracer_id]["pos"] = np.array(
                        [
                            temp_array[
                                :, np.where(val_indices == 1)[0][0] * len(ids) + j
                            ],
                            temp_array[
                                :, np.where(val_indices == 2)[0][0] * len(ids) + j
                            ],
                            temp_array[
                                :, np.where(val_indices == 3)[0][0] * len(ids) + j
                            ],
                        ]
                    ).T
                    tracer[tracer_id][val] = tracer[tracer_id][val][
                        : self.NTimesteps, :
                    ]

                elif val == "vel":
                    tracer[tracer_id]["vel"] = np.array(
                        [
                            temp_array[
                                :, np.where(val_indices == 7)[0][0] * len(ids) + j
                            ],
                            temp_array[
                                :, np.where(val_indices == 8)[0][0] * len(ids) + j
                            ],
                            temp_array[
                                :, np.where(val_indices == 9)[0][0] * len(ids) + j
                            ],
                        ]
                    ).T
                    tracer[tracer_id][val] = tracer[tracer_id][val][
                    ...
                    tracer[tracer_id]["xnuc"] = np.asarray(
                        [
                            temp_array[
                                :, np.where(val_indices == sp)[0][0] * len(ids) + j
                            ]
                            for sp in self.vals_dict["xnuc"]
                        ]
                    ).T
                    tracer[tracer_id][val] = tracer[tracer_id][val][
                        : self.NTimesteps, :
                    ...
                    tracer[tracer_id][val] = temp_array[
                        :,
                        np.where(val_indices == self.vals_dict[val])[0][0] * len(ids)
                        + j,
                    ].T
    ...
    def access_val(self, val):
        ...
        array_of_dicts = np.array(
            [self.followed_tracers[key] for key in self.followed_tracers.keys()]
        )
    ...
    def add_followed_tracers(self, filename="followed_tracers", n=-1, path=-1):
        ...
        print(
            "We have {:d} tracers that are in both datasets".format(
                len(old_ids) - len(ids_to_add)
            )
        )
        start = time.time()

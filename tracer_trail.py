...
def plot_tracer(NTimestep, pos, box=0.3, center=3.6e11, axes=[0, 1], trail_length=4, binary=False,centerofmass=False):
    ...
    box_center = np.ones(3)*params["BoxSize"]/2.
    ...
    if len(axes)>2:
        ...
        if not centerofmass is False:
            ax[0].scatter(centerofmass[NTimestep,0],centerofmass[NTimestep,1],marker='+',s=30,c='r',zorder=1000)
            ax[1].scatter(centerofmass[NTimestep,0],centerofmass[NTimestep,2],marker='+',s=30,c='r',zorder=1000)
        ax[0].scatter(box_center[0],box_center[1],marker='+',s=30,c='g',zorder=1000)
        ax[1].scatter(box_center[0],box_center[2],marker='+',s=30,c='g',zorder=1000)
        for tr in range(pos.shape[0]):
            ax[0].plot(pos[tr,i_in:NTimestep,0],pos[tr,i_in:NTimestep,1],c='w',lw=1)
            ax[1].plot(pos[tr,i_in:NTimestep,0],pos[tr,i_in:NTimestep,2],c='w',lw=1)
            ...
            ax[1].text(0.7, 0.93, 'Time: {:>7.02f}s'.format(current_time), transform=ax[1].transAxes, fontname='Miriam Libre', color='white')
    else:
        fig, ax = plt.subplots(1,1)
        for tr in range(pos.shape[0]):
            ax.plot(pos[tr,i_in:NTimestep,axes[0]],pos[tr,i_in:NTimestep,axes[1]],c='w',lw=1)
        ax.scatter(centerofmass[NTimestep,axes[0]],centerofmass[NTimestep,axes[1]],marker='+',s=30,c='r',zorder=1000)
        ax.scatter(box_center[axes[0]],box_center[axes[1]],marker='+',s=30,c='g',zorder=1000)
        ax.text(0.7, 0.93, 'Time: {:>7.02f}s'.format(NTimestep*tracer.tracer_dt), transform=ax.transAxes, fontname='Miriam Libre', color='white')
...
center = [3.664e+11, 3.6625e+11, 3.65461144e+11]
startfrom = 0
n_tracers = 1000
    ...
    for flag in sys.argv[2:]:
        ...
        try:
            if (not os.path.isdir(flag)):
                if is_master():
                    yn = input(flag+" does not exist, do you want to create it? [Y/n]\n")
                    if yn == 'Y':
                        os.mkdir(flag)
                    else:
...
if os.path.isfile(os.path.join(base_path,"followed_tracers"+str(n_tracer_file).zfill(2))):
    tracer.read_followed_tracers(path=base_path,n=n_tracer_file)
else:
    if is_master():
        tracer.find_tracers_at_time(0)
        ids = tracer.find_tracers_in_box([3e10,3e10,1e10],[3.65461144e+11, 3.65461144e+11, 3.65461144e+11])
...
if NTimesteps.size>0:
    if mpi_size() > len(NTimesteps):
        if is_master():
            ...
            plot_(NTimesteps, pos, box=boxsize,center=center,axes=axes,binary=binary,centerofmass=centerofmass)
    else:
        plot_tracer(NTimesteps, pos, box=boxsize,center=center,axes=axes,binary=binary,centerofmass=centerofmass)
mpi_barrier()
